...
public class AirportSpecs {
    ...
    public ChoreographySpec getChorSpec() {
        ...
        DeployableServiceSpec[] serviceArray = new DeployableServiceSpec[services.size()];
    ...
    private void createBusSpec() {
        createServiceSpec(AIRPORT_BUS_COMPANY, AIRPORT_BUS_COMPANY_JAR, AIRPORT_BUS_COMPANY_PORT);
    }

    private void createDisplaySpec() {
        createServiceSpec(AIRPORT_DISPLAY_ACTUATORS_AGGREGATOR, AIRPORT_DISPLAY_ACTUATORS_AGGREGATOR_JAR,
                AIRPORT_DISPLAY_ACTUATORS_AGGREGATOR_PORT);
    }

    private void createInfraredSpec() {
        createServiceSpec(AIRPORT_INFRARED_SENSORS_AGGREGATOR, AIRPORT_INFRARED_SENSORS_AGGREGATOR_JAR,
                AIRPORT_INFRARED_SENSORS_AGGREGATOR_PORT);
    }

    private void createNoiseSpec() {
        createServiceSpec(AIRPORT_NOISE_SENSORS_AGGREGATOR, AIRPORT_NOISE_SENSORS_AGGREGATOR_JAR,
                AIRPORT_NOISE_SENSORS_AGGREGATOR_PORT);
    }

    private void createPresssureSpec() {
        createServiceSpec(AIRPORT_PRESSURE_SENSORS_AGGREGATOR, AIRPORT_PRESSURE_SENSORS_AGGREGATOR_JAR,
                AIRPORT_PRESSURE_SENSORS_AGGREGATOR_PORT);
    }

    private void createSignSpec() {
        createServiceSpec(AIRPORT_SIGN_ACTUATORS_AGGREGATOR, AIRPORT_SIGN_ACTUATORS_AGGREGATOR_JAR,
                AIRPORT_SIGN_ACTUATORS_AGGREGATOR_PORT);
    }

    private void createSpeakerSpec() {
        createServiceSpec(AIRPORT_SPEAKER_ACTUATORS_AGGREGATOR, AIRPORT_SPEAKER_ACTUATORS_AGGREGATOR_JAR,
                AIRPORT_SPEAKER_ACTUATORS_AGGREGATOR_PORT);
    }

    private void createAmenitySpec() {
        createServiceSpec(BOOKABLE_AMENITY, BOOKABLE_AMENITY_JAR, BOOKABLE_AMENITY_PORT);
    }

    private void createLuggageSpec() {
        createServiceSpec(LUGGAGE_HANDLING_COMPANY, LUGGAGE_HANDLING_COMPANY_JAR, LUGGAGE_HANDLING_COMPANY_PORT);
        addDependency(AIRPORT);
    }

    private void createMidDisplaySpec() {
        createServiceSpec(MID_DISPLAY_ACTUATORS_AGGREGATOR, MID_DISPLAY_ACTUATORS_AGGREGATOR_JAR,
                MID_DISPLAY_ACTUATORS_AGGREGATOR_PORT);
    }

    private void createMidLocationSpec() {
        createServiceSpec(MID_LOCATION_SENSORS_AGGREGATOR, MID_LOCATION_SENSORS_AGGREGATOR_JAR,
                MID_LOCATION_SENSORS_AGGREGATOR_PORT);
    }

    private void createMidMicSpec() {
        createServiceSpec(MID_MICROPHONE_SENSORS_AGGREGATOR, MID_MICROPHONE_SENSORS_AGGREGATOR_JAR,
                MID_MICROPHONE_SENSORS_AGGREGATOR_PORT);
    }

    private void createSecuritySpec() {
        createServiceSpec(SECURITY_COMPANY, SECURITY_COMPANY_JAR, SECURITY_COMPANY_PORT);
        addDependency(AIRPORT);
    }

    private void createStandServiceSpec() {
        createServiceSpec(STAND_AND_GATE_MANAGEMENT, STAND_AND_GATE_MANAGEMENT_JAR, STAND_AND_GATE_MANAGEMENT_PORT);
        addDependency(AIRPORT);
    }
    ...
    private void createServiceSpec(final String name, final String jar, int port) {
        service = new DeployableServiceSpec(name, ServiceType.SOAP, PackageType.COMMAND_LINE, resourceImpact, VERSION,
                jar, port, name, 1);
        services.add(service);
    }

    private void addDependency(final String depServiceName) {
        final ServiceDependency dependency = new ServiceDependency(depServiceName, depServiceName);
